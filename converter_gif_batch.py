...
def main():
    ...
    if messagebox.askyesno("Batch mode", "Convert all .webm files in a folder?"):
        in_dir = filedialog.askdirectory(
            title="Select folder containing .webm files",
            initialdir=(
        ...
        # Settings (ask once, apply to all files)
        fps = ask_number(
            "Frames per Second",
        ...
        files = [f for f in sorted(os.listdir(in_dir)) if f.lower().endswith(".webm")]
        if not files:
            messagebox.showwarning(
                "No files", "No .webm files found in the selected folder."
            )
            return

        messagebox.showinfo("Converting", f"Converting {len(files)} file(s)…")
        ok = 0
        errors = []
        ...
        for fname in files:
            ...
            out_path = os.path.join(out_dir, base + ".gif")
            try:
                make_gif_im(
                    in_path,
                    out_path,
                    fps=fps,
                    max_w=max_w,  # conditional scaler inside make_gif_im avoids upscaling
                    dither="Riemersma",  # or "FloydSteinberg"
                )
                ok += 1
            except subprocess.CalledProcessError as e:
                msg = e.stderr.strip() if e.stderr else str(e)
                errors.append(f"{fname}: {msg.splitlines()[-1] if msg else 'error'}")
        ...
        if errors:
            messagebox.showwarning(
                ...
                f"Converted {ok}/{len(files)} files to:\n{out_dir}\n\nFirst error:\n{errors[0]}",
            )
        else:
            messagebox.showinfo("Done", f"Converted {ok} file(s) to:\n{out_dir}")
